import {getMenu,getSiteInfo,getFriendLink,getIndexBanner,getTags,getComments} from '@/api'
...
const actions = {
  ...
  GET_FriendLink ({ commit, state }, params) {
    return new Promise((resolve, reject) => {
      getFriendLink(params).then((data) => {
  ...
  GET_BANNER ({ commit, state }, params) {
    return new Promise((resolve, reject) => {
      getIndexBanner(params).then((data) => {
