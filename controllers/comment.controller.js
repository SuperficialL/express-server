...
const { arrayUniq, numberIsInvalid, arrayIsInvalid, objectValues } = require("../utils/tools");
const { COMMENT_STATE, COMMENT_POST_TYPE, SORT_TYPE } = require("../core/constants");
const {
  handleError,
  handleSuccess,
...
const updateArticleCommentCount = (article_ids = []) => {
  ...
  Comment.aggregate([
    { $match: { state: COMMENT_STATE.published, article_id: { $in: article_ids } } },
    ...
    .then(counts => {
      if (arrayIsInvalid(counts)) {
        ...
        counts.forEach(count => {
          Article.updateOne({ id: count._id }, { $set: { "comments": count.num_tutorial } })
            .then(info => {
              consola.info("评论聚合更新成功", info)
            })
            .catch(err => {
...
const sendMailToAdminAndTargetUser = (comment, permalink) => {
  ...
  const commentType = comment.article_id === COMMENT_POST_TYPE.guestbook ? "评论" : "留言";
  ...
  const sendMailHtml = isReply => `<p> 来自 ${comment.author.name} 的${commentType}${replyText(isReply)}：${comment.content}</p><br><a href="${permalink}" target="_blank">[ 点击查看 ]</a>`;
...
CommentCtrl.list.POST = (req, res) => {
  ...
  const doSaveComment = () => {
    ...
    // 永久链接
    const pageLink = comment.article_id === COMMENT_POST_TYPE.guestbook ? "guestbook" : `detail/${comment.article_id}`;
  ...
  queryIp(ip).then(({ body: { resultcode, result } }) => {
    if (resultcode == 200) {
      ip_location = {
        city: result.City,
...
// 修改单个评论
CommentCtrl.item.PATCH = ({ params: { comment_id }, body: comment, headers: { referer } }, res) => {
